...
namespace Eagle.Core.Pagination.Extension
{
    public static class PagedListPagerHtmlHelper
    {
        ...
        {
            var targetPageNumber = list.TotalPageCount;
        ...
        private static TagBuilder PageCountAndLocationText(IPagedList list, PagedListRenderOptions options)
        {
            ...
            text.SetInnerText(string.Format(options.PageCountAndCurrentLocationFormat, list.PageNumber, list.TotalPageCount));
        ...
        {
            if (options.Display == PagedListDisplayMode.Never || (options.Display == PagedListDisplayMode.IfNeeded && list.TotalPageCount <= 1))
                return null;
            ...
            var lastPageToDisplay = list.TotalPageCount;
            var pageNumbersToDisplay = lastPageToDisplay;
            if (options.MaximumPageNumbersToDisplay.HasValue && list.TotalPageCount > options.MaximumPageNumbersToDisplay)
            {
                ...
                if (lastPageToDisplay > list.TotalPageCount)
                    firstPageToDisplay = list.TotalPageCount - maxPageNumbersToDisplay + 1;
            }
            ...
            if (options.DisplayLinkToIndividualPages)
            {
                ...
                if (options.DisplayEllipsesWhenNotShowingAllPageNumbers && (firstPageToDisplay + pageNumbersToDisplay - 1) < list.TotalPageCount)
                    listItemLinks.Add(Ellipses(options));
            }
            ...
            //last
            if (options.DisplayLinkToLastPage == PagedListDisplayMode.Always || (options.DisplayLinkToLastPage == PagedListDisplayMode.IfNeeded && lastPageToDisplay < list.TotalPageCount))
        ...
        public static MvcHtmlString PagedListGoToPageForm(this System.Web.Mvc.HtmlHelper html, IPagedList list, string formAction,
            GoToFormRenderOptions options, AjaxOptions ajaxOptions, object htmlAttributes = null)
        {
            var form = new TagBuilder("form");
            form.AddCssClass("form");
            form.Attributes.Add("method", "get");

            var fieldset = new TagBuilder("div");
            fieldset.AddCssClass("row form-inline");
            ...
            input.Attributes.Add("type", options.InputFieldType);
            input.Attributes.Add("id", options.InputFieldName);
            input.Attributes.Add("name", options.InputFieldName);
            input.Attributes.Add("value", list.PageNumber.ToString());
            input.Attributes.Add("pattern", @"\d+");
            input.Attributes.Add("min", (options.Min != null)?Convert.ToString(options.Min) :"1");
            input.Attributes.Add("max", (options.Max != null) ? Convert.ToString(options.Max) : "999");
            input.MergeAttribute("style", "width:80px");

            var aTagBuilder = new TagBuilder("a");
            aTagBuilder.AddCssClass("btn btn-primary btn-sm");
            aTagBuilder.GenerateId("goBtn");

            if (htmlAttributes != null)
            {
                var attributes = System.Web.Mvc.HtmlHelper.AnonymousObjectToHtmlAttributes(htmlAttributes);
                aTagBuilder.MergeAttributes(attributes, true);
            }
            ...
            {
                aTagBuilder.Attributes.Add("href", formAction);
            }
            
            aTagBuilder.SetInnerText(options.SubmitButtonFormat);

            fieldset.InnerHtml = $"{label} {input} {aTagBuilder}";
            form.InnerHtml = fieldset.ToString();
            return new MvcHtmlString(form.ToString());
        }
    }
