CREATE PROCEDURE [dbo].[Menu_GetParentNodesOfSelectedNodeByMenuId]
(@MenuId int)
AS
BEGIN
	WITH Hierarchy(PageId, TypeId, MenuId, MenuCode, ParentId, Depth, Lineage, ListOrder, MenuName, 
                         MenuTitle, MenuAlias,  Description, Target,  IconFile, IconClass, Color, CssClass, MenuStatus, 
	...
	(
		...
		FROM            [dbo].[Menu] as m INNER JOIN
                         [dbo].[Page] as p ON m.PageId = p.PageId WHERE m.MenuId = @MenuId 
