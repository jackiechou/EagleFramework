    ...
    public class MessageQueueHelper<T> : MarshalByRefObject where T : class, new()
    {
        ...
        public MessageQueueHelper(string path)
        {
            ...
            else
            {
                ...
                m_Msq.MaximumQueueSize = queueMaxSize;
            }
        ...
        public T Receive()
        {
            ...
            System.Messaging.Message m = m_Msq.Receive(MessageQueueTransactionType.Single);
            return m.Body as T;
        }
        public IList<T> ReceiveAll()
        {
            if (!MssageQueueReady()) return null;
            System.Messaging.Message[] ms = m_Msq.GetAllMessages();
        ...
        public T Peek()
        {
            ...
            System.Messaging.Message m = m_Msq.Peek();
            return m.Body as T;
        }
