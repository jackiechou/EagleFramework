        ...
        public string ListenHubEvent(Action<object, BroadCastEventArgs> hubEvent)
        {
            ...
            catch (Exception exception)
            {
                ...
                if (hubConnection != null && !string.IsNullOrWhiteSpace(hubConnection.ConnectionId))
                    return
                        $"Opps something wrong happened ! Hub ConnectionID : {hubConnection.ConnectionId}, Exception - Message : {exception.Message}, StackTrace : {exception.StackTrace} ";
                return
                    $"Opps something wrong happened ! Exception - Message : {exception.Message}, StackTrace : {exception.StackTrace} ";
            }
